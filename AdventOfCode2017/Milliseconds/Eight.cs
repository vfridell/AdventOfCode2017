...
namespace AdventOfCode2017.Milliseconds
{
    public class Eight
    {
        public static void Part1and2()
        {
            ...
            Ident maxRegister = Registers.Values.OrderByDescending(i => i.Value).First();
            Console.WriteLine($"Max Register is {maxRegister.Name} with value {maxRegister.Value}");
            Console.WriteLine($"Max value in any Register ever was {TotalMax}");
        }

        private static Dictionary<string, Ident> Registers = new Dictionary<string, Ident>();
        ...
        public class Statement
        {
            ...
            public void Execute()
            {
                if (Comparison.ResultFunction())
                {
                    Operation.ResultAction();
                    if (Eight.TotalMax < Operation.Ident.Value) Eight.TotalMax = Operation.Ident.Value;
                }
            }
        }
        ...
        public class Comparison
        {
            ...
            public Func<bool> ResultFunction;
        ...
        private static Operation GetOperation(string inputString)
        {
            Regex opRegex = new Regex("([a-z]+) (inc|dec) (-?[0-9]+)");
            ...
            {
                targetIdent = new Ident() { Name = name, Value = 0 };
                Registers.Add(targetIdent.Name, targetIdent);
            }

            Operation op = new Operation() { Ident = targetIdent, Literal = literal };

            if (mc[0].Groups[2].Value == "inc")
                op.ResultAction = () => { targetIdent.Value += literal; };
            else
                op.ResultAction = ()  => { targetIdent.Value -= literal; };
        ...
        private static Comparison GetComparison(string inputString)
        {
            ...
            Comparison comp = new Comparison() { Ident = targetIdent, Literal = literal };
        ...
        public static List<string> inputtest = new List<string>() { "b inc 5 if a > 1",
"a inc 1 if b < 5",
"c dec -10 if a >= 1",
"c inc -20 if c == 10",
};

        public static List<string> input = new List<string>() { "g dec 231 if bfx > -10",
"k dec -567 if wfk == 0",
"jq inc 880 if a < 2",
"sh inc -828 if nkr < -5",
"w inc 595 if nkr > -10",
